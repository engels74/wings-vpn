...
- `router/` — gin HTTP API (`router/router.go` `Configure` wires all routes); handlers live in
  `router/router_server_*.go`; `router/websocket/` is the console WS.
...
Requires Go 1.25+. The binary only builds/runs on Linux (it drives Docker); on macOS pin
`GOOS=linux` for builds.

```bash
go build ./...                       # build all packages (any OS)
env GOOS=linux CGO_ENABLED=0 go build ./...   # macOS: build the Linux-only tree (matches CI/prek)
...
gofumpt -w .                         # format (stricter gofmt; provided in nix devShell)
golangci-lint run                    # meta-linter (provided in nix devShell)
make build                           # cross-compile linux amd64+arm64 into build/
```

CI (`.github/workflows/push.yaml`) runs on `ubuntu-22.04` across Go `1.25.7`/`1.26.0`:
`go mod download`, build (linux, amd64+arm64, `CGO_ENABLED=0`), `go test` then `go test -race`
(`CGO_ENABLED=1`, amd64 only). CodeQL also runs.

Pre-commit is configured via `prek.toml`: conventional-commit messages are enforced, direct
commits to `main` are blocked, and local hooks run `gofumpt -w` plus a `GOOS=linux` `go build`.
...
2. Implement the handler in the relevant `router/router_server_*.go` file; pull the server with
   the existing middleware-provided context (don't re-fetch).
...
| JSON encode/decode | `github.com/goccy/go-json` (the project default) | `encoding/json` in hot paths |
...
Container network mode detection lives on the config struct (`config/config_docker.go`); `Mode`
maps to the `network_mode` YAML key:
...
- When editing `environment/docker/container.go` `Create`, guard network-related fields with
  ...
  interface-IP rewrite, `hostname`/`domainname`, DNS, port bindings, and macvlan endpoint
  config. Setting any of these in that mode causes a Docker API error or binds to a
  non-existent address.
- `EnvironmentVariables()` returns the Configuration's **shared backing slice**. Before
  mutating it (e.g. the `SERVER_IP` rewrite) call `slices.Clone(...)` first (see
  `container.go` `Create`, ~line 166) — mutating in place races with concurrent readers and
  persists rewritten values into stored config.
...
- `README.md` — fork overview, example `config.yml`, links to upstream Pelican docs.
